...
`func`がプログラムを停止せずに制御を戻した場合、
プログラムはシグナルに依る割り込みが発生した点から再開する。
...
`signal`によって登録されシグナルによって実行された関数および
その関数から再帰的に呼び出される(標準ライブラリ関数以外の)関数を
シグナルハンドラと呼ぶ。

`SIGFPE`, `SIGILL`, `SIGSEGV` および処理系定義のシグナルによって直接呼び出され
たシグナルハンドラ関数`func`がプログラムを停止せずに制御を戻した場合は未定義の動作になる。

直接登録されるシグナルハンドラ関数`func`は戻り値を持たず、`int`型の引数を持つ。
この引数にはシグナル番号が格納される。

`abort`または`signal`の呼び出しによって実行されたシグナルハンドラ内で
`raise`関数を呼び出してはならない。
...
それ以外の場合は`SIG_ERR`を返し、正の値が`errno`に設定される。

## 備考

関数`func`がシグナル`sig`に対して実行されている間に別のシグナルが発生したときは処理系定義である。
現在の`func`が終了するまで、
処理系はあたかも `std::signal(sig, SIG_DFL)` が設定されているかのように`sig`に対して既定の処理を行っても良いし、
または現在のシグナル`sig`を含む複数の処理系定義のシグナル発生を阻止しても良い。
現在のシグナル`sig`が`SIGILL`のときは代わりに何もしないように設定しても良い。

プログラム開始時に処理系定義のシグナルの集合に対して

```cpp
std::signal(sig, SIG_IGN);
```

が呼び出され、それ以外のシグナルの集合に対して
...
処理系はあたかも標準ライブラリ関数が内部的に`signal`を呼び出さないかのように振る舞う必要がある。
